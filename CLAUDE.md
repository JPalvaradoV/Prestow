...
**Actualizado el 22 de septiembre de 2026**, con la configuración vigente (capacidad real por plan + warm start de las pasadas 1-3, ver sección 6). Re-verificado con el test de estabilidad de 5 corridas (pendiente #1 de la sección 10, `data/stability_report.csv`): estos números son **idénticos en las 5 corridas**, no una muestra suelta.
...
| Desbalance entre cuadrillas | 6,9% | **9,36%** | peor |
| Fragmentación | 14 bodegas | **10** | mejor |
...
**Pendiente:** la validación histórica "el modelo reproduce las horas del archivo del puerto con error de 0,01 h en las cuatro cuadrillas" se hizo con el 58,19 h anterior (antes de la sección 6) y no se volvió a correr contra el modelo vigente.
...
4. **La pasada 2 no resuelve en 150-180 s** — probablemente resuelto de forma indirecta el 22-sep-2026 al agregarle warm start desde la solución de la pasada 1 (ver sección 6): en la corrida de verificación a 180 s la pasada 2 llegó a gap 3,24%, mucho mejor que el estado sin warm start — y se repitió el mismo 3,24% exacto en las 5 corridas del test de estabilidad (ver ítem 1), así que no es un resultado suelto. Aun así no se hizo una comparación A/B controlada (misma corrida, con y sin warm start en la pasada 2) para confirmarlo con certeza — dejarlo como pendiente de verificación, no cerrarlo del todo sin esa comparación.
...
7. **La pasada 3 (balance de peso) nunca prueba optimalidad dentro del tiempo estándar** (gap 63,91% en el test de estabilidad del 22-sep-2026, ítem 1 — antes se había visto un rango de 16-55% en pruebas previas, con otra configuración del modelo). **Actualización 22-sep-2026: con la configuración vigente (capacidad real por plan + warm start), el gap de la pasada 3 SÍ salió idéntico en las 5 corridas del test de estabilidad** — la variabilidad entre corridas que motivó este ítem originalmente (documentada como 4.930 t vs 9.152 t de desbalance en dos corridas, sección 10 de `docs/05_Estado_app_web.md`) se midió con una configuración anterior del modelo y no se reprodujo hoy. Sigue sin probar optimalidad (gap alto), pero eso ya no implica que el resultado varíe entre corridas — no está claro todavía si dejó de variar por el warm start, por la capacidad real por plan, o por otra razón; no investigado a fondo.
