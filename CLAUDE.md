...
7. **La pasada 3 (balance de peso) no optimizaba nada — investigado el 25-sep-2026, arreglo pendiente de decisión.** El gap de ~64% NO era una relajación débil: HiGHS devolvía sin cambios su punto de partida (la solución de la pasada 2), que además traía `peso_min = 0` — el "desbalance" de 9.068 t era el peso de la bodega más cargada; el real era 6.294 t. Corregido el punto de partida, HiGHS igual no mejora en 180 s. Una búsqueda por vecindarios (liberar un par de bodegas a la vez, fijar el resto) llega a **3.604 t en ~30 s** (gap ~9% contra la cota dual 3.272 t), mismo makespan, 0 violaciones, y de paso baja el desbalance entre cuadrillas de 9,4% a 5,8%. Detalle en `docs/05_Estado_app_web.md`, sección 20.
